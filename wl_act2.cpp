...
dirType_t dirtable[9] = {northWest,north,northEast,west,noDir,east,
...
qint8 ProjectileTryMove (objtype *ob)
{
    int      xl,yl,xh,yh,x,y;
    objtype *check;
...
void T_Projectile (objtype *ob)
{
    ...
    if (!ProjectileTryMove (ob))
    {
        if (ob->obclass == rocketobj) {
            PlaySoundLocActor(wlcache->findSound("MISSILEHITSND"), ob);
            ob->state = &s_boom1;
        } else
    ...
    if (deltax < PROJECTILESIZE && deltay < PROJECTILESIZE) {
        switch (ob->obclass) {
        case needleobj:
            damage = (wluser->US_RndT() >>3) + 20;
            break;
        case rocketobj:
        case hrocketobj:
        case sparkobj:
            damage = (wluser->US_RndT() >>3) + 30;
            break;
        case fireobj:
            damage = (wluser->US_RndT() >>3);
            break;
        default:
            break;
        }
...
void SpawnStand (enemy_t which, int tilex, int tiley, int dir)
{
    ...
    switch (which) {
    case en_guard:
        wlstate->spawnNewObj(tilex,tiley,&s_grdstand);
    ...
    case en_officer:
        wlstate->spawnNewObj(tilex,tiley,&s_ofcstand);
        newobj->speed = SPDPATROL;
        if (!loadedgame)
            gamestate.killtotal++;
        break;
    ...
    case en_mutant:
        wlstate->spawnNewObj(tilex,tiley,&s_mutstand);
        newobj->speed = SPDPATROL;
        if (!loadedgame)
            gamestate.killtotal++;
        break;
    ...
    case en_ss:
        wlstate->spawnNewObj(tilex,tiley,&s_ssstand);
        newobj->speed = SPDPATROL;
        if (!loadedgame)
            gamestate.killtotal++;
        break;
    ...
    newobj->dir = (dirType_t)(dir * 2);
    newobj->flags |= FL_SHOOTABLE;
}

void SpawnDeadGuard (int tilex, int tiley)
{
    wlstate->spawnNewObj(tilex,tiley,&s_grddie4);
    DEMOIF_SDL
    {
...
{
    wlstate->spawnNewObj(tilex,tiley,&s_bossstand);
    newobj->speed = SPDPATROL;
...
{
    wlstate->spawnNewObj(tilex,tiley,&s_gretelstand);
    newobj->speed = SPDPATROL;
...
void SpawnPatrol (enemy_t which, int tilex, int tiley, int dir)
{
    switch (which) {
    case en_guard:
        wlstate->spawnNewObj(tilex,tiley,&s_grdpath1);
        newobj->speed = SPDPATROL;
        if (!loadedgame)
            gamestate.killtotal++;
        break;
    ...
    case en_officer:
        wlstate->spawnNewObj(tilex,tiley,&s_ofcpath1);
        newobj->speed = SPDPATROL;
        if (!loadedgame)
            gamestate.killtotal++;
        break;
    ...
    case en_ss:
        wlstate->spawnNewObj(tilex,tiley,&s_sspath1);
        newobj->speed = SPDPATROL;
        if (!loadedgame)
            gamestate.killtotal++;
        break;
    ...
    case en_mutant:
        wlstate->spawnNewObj(tilex,tiley,&s_mutpath1);
        newobj->speed = SPDPATROL;
        if (!loadedgame)
            gamestate.killtotal++;
        break;
    ...
    case en_dog:
        wlstate->spawnNewObj(tilex,tiley,&s_dogpath1);
        newobj->speed = SPDDOG;
        if (!loadedgame)
            gamestate.killtotal++;
        break;
...
void A_DeathScream (objtype *ob)
{
    switch (ob->obclass) {
    case mutantobj:
        PlaySoundLocActor(wlcache->findSound("AHHHGSND"), ob);
        break;

    case guardobj:
    {
        int sounds[9]={ wlcache->findSound("DEATHSCREAM1SND"),
                        wlcache->findSound("DEATHSCREAM2SND"),
                        wlcache->findSound("DEATHSCREAM3SND"),
                        wlcache->findSound("DEATHSCREAM4SND"),
                        wlcache->findSound("DEATHSCREAM5SND"),
                        wlcache->findSound("DEATHSCREAM7SND"),
                        wlcache->findSound("DEATHSCREAM8SND"),
                        wlcache->findSound("DEATHSCREAM9SND")
                      };
        PlaySoundLocActor(sounds[wluser->US_RndT() % 2],ob);
        break;
    }
    case officerobj:
        PlaySoundLocActor(wlcache->findSound("NEINSOVASSND"), ob);
        break;
    case ssobj:
        PlaySoundLocActor(wlcache->findSound("LEBENSND"), ob); // JAB
        break;
    case dogobj:
        PlaySoundLocActor(wlcache->findSound("DOGDEATHSND"), ob); // JAB
        break;
    case bossobj:
        wlaudio->playSound("MUTTISND"); // JAB
        break;
    case schabbobj:
        wlaudio->playSound("MEINGOTTSND");
        break;
    case fakeobj:
        wlaudio->playSound("HITLERHASND");
        break;
    case mechahitlerobj:
        wlaudio->playSound("SCHEISTSND");
        break;
    case realhitlerobj:
        wlaudio->playSound("EVASND");
        break;
    case gretelobj:
        wlaudio->playSound("MEINSND");
        break;
    case giftobj:
        wlaudio->playSound("DONNERSND");
        break;
    case fatobj:
        wlaudio->playSound("ROSESND");
        break;
    default:
        break;
    }
...
void SpawnGhosts (int which, int tilex, int tiley)
{
    ...
    case en_blinky:
        ...
        break;
    case en_clyde:
        wlstate->spawnNewObj(tilex,tiley,&s_clydechase1);
        break;
    case en_pinky:
        wlstate->spawnNewObj(tilex,tiley,&s_pinkychase1);
        break;
    case en_inky:
        wlstate->spawnNewObj(tilex,tiley,&s_inkychase1);
        break;
    }
...
void SpawnSchabbs (int tilex, int tiley)
{
    ...
    wlstate->spawnNewObj(tilex,tiley,&s_schabbstand);
    newobj->speed = SPDPATROL;
...
void SpawnGift (int tilex, int tiley)
{
    ...
    wlstate->spawnNewObj(tilex, tiley, &s_giftstand);
    newobj->speed = SPDPATROL;
...
void SpawnFat (int tilex, int tiley)
{
    ...
    wlstate->spawnNewObj(tilex,tiley,&s_fatstand);
    newobj->speed = SPDPATROL;
...
void T_SchabbThrow (objtype *ob)
{
    ...
    newobj->flags = FL_NEVERMARK;
    newobj->active = ac_yes;
    PlaySoundLocActor (wlcache->findSound("SCHABBSTHROWSND"), newobj);
}
...
void T_GiftThrow (objtype *ob)
{
    ...
    newobj->flags = FL_NEVERMARK;
    newobj->active = ac_yes;
    PlaySoundLocActor(wlcache->findSound("MISSILEFIRESND"), newobj);
}
...
void T_Schabb (objtype *ob)
{
    ...
    if (wlstate->checkLine(ob)) {
        ob->hidden = false;
        if ( (unsigned) wluser->US_RndT() < (tics<<3) && objfreelist) {
            wlstate->newState(ob,&s_schabbshoot1);
            return;
        }
        dodge = true;
    } else
    ...
    while (move) {
        if (ob->distance < 0) {
            wlitem->openDoor(-ob->distance - 1);
            if (wlitem->doorObjList(-ob->distance-1)->action != dr_open)
                return;
        ...
        ob->x = ((qint32)ob->tilex<<TILESHIFT)+TILEGLOBAL/2;
        ob->y = ((qint32)ob->tiley<<TILESHIFT)+TILEGLOBAL/2;
        move -= ob->distance;
        if (dist <4)
            wlstate->selectRunDir(ob);
        else if (dodge)
            wlstate->selectDodgeDir(ob);
        else
            wlstate->selectChaseDir(ob);
...
void T_Gift (objtype *ob)
{
    ...
    if (wlstate->checkLine(ob)) {
        ob->hidden = false;
        if ( (unsigned) wluser->US_RndT() < (tics<<3) && objfreelist) {
            wlstate->newState(ob,&s_giftshoot1);
            return;
        }
        dodge = true;
    } else
...
void T_Fat (objtype *ob)
{
    ...
    if (wlstate->checkLine(ob)) {
        ob->hidden = false;
        if ( (unsigned) wluser->US_RndT() < (tics<<3) && objfreelist) {
            wlstate->newState(ob,&s_fatshoot1);
            return;
        }
        dodge = true;
    } else
...
void SpawnFakeHitler (int tilex, int tiley)
{
    ...
    wlstate->spawnNewObj(tilex,tiley,&s_fakestand);
    newobj->speed = SPDPATROL;
...
void SpawnHitler (int tilex, int tiley)
{
    ...
    wlstate->spawnNewObj(tilex,tiley,&s_mechastand);
    newobj->speed = SPDPATROL;
...
void A_HitlerMorph (objtype *ob)
{
    ...
    wlstate->spawnNewObj(ob->tilex,ob->tiley,&s_hitlerchase1);
...
void A_MechaSound (objtype *ob)
{
    if (*wlitem->areaByPlayer(ob->areanumber))
        PlaySoundLocActor(wlcache->findSound("MECHSTEPSND"), ob);
}
...
void T_FakeFire (objtype *ob)
{
    ...
    if (!objfreelist) {
        wlstate->newState(ob,&s_fakechase1);
        return;
    }
    deltax = player->x - ob->x;
    deltay = ob->y - player->y;
    ...
    newobj->flags = FL_NEVERMARK;
    newobj->active = ac_yes;
    PlaySoundLocActor (wlcache->findSound("FLAMETHROWERSND"), newobj);
}
...
void T_Fake (objtype *ob)
{
    ...
    if (wlstate->checkLine(ob)) {
        ob->hidden = false;
        if ( (unsigned) wluser->US_RndT() < (tics<<1) && objfreelist) {
            wlstate->newState(ob,&s_fakeshoot1);
            return;
        }
    } else
        ob->hidden = true;
...
void T_Chase (objtype *ob)
{
    ...
    if (wlstate->checkLine(ob)) {
        ...
        if ( wluser->US_RndT()<chance) {
            switch (ob->obclass) {
            case guardobj:
                wlstate->newState(ob,&s_grdshoot1);
                break;
            case officerobj:
                wlstate->newState(ob,&s_ofcshoot1);
                break;
            case mutantobj:
                wlstate->newState(ob,&s_mutshoot1);
                break;
            case ssobj:
                wlstate->newState(ob,&s_ssshoot1);
                break;
            case bossobj:
                wlstate->newState(ob,&s_bossshoot1);
                break;
            case gretelobj:
                wlstate->newState(ob,&s_gretelshoot1);
                break;
            case mechahitlerobj:
                wlstate->newState(ob,&s_mechashoot1);
                break;
            case realhitlerobj:
                wlstate->newState(ob,&s_hitlershoot1);
                break;
            default:
                break;
            }
...
void T_DogChase (objtype *ob)
{
    ...
    while (move) {
        ...
        if (dx <= MINACTORDIST) {
            ...
            dy -= move;
            if (dy <= MINACTORDIST) {
                wlstate->newState(ob,&s_dogjump1);
                return;
            }
        }
        if (move < ob->distance) {
...
void SelectPathDir (objtype *ob)
{
    ...
    spot = wlcache->mapSpot(ob->tilex,ob->tiley,1)-ICONARROWS;

    if (spot<8) {
        // new direction
        ob->dir = (dirType_t)(spot);
    }
    ob->distance = TILEGLOBAL;
    if (!wlstate->tryWalk(ob))
        ob->dir = noDir;
}
...
void T_Shoot (objtype *ob)
{
    ...
    if (wlstate->checkLine(ob)) {
        ...
        if (wluser->US_RndT() < hitchance) {
            if (dist < 2)
                damage = wluser->US_RndT() >> 2;
            else if (dist < 4)
                damage = wluser->US_RndT() >> 3;
            else
    ...
    switch(ob->obclass) {
    case ssobj:
        PlaySoundLocActor(wlcache->find("SSFIRESND"), ob);
        break;
    case giftobj:
    case fatobj:
        PlaySoundLocActor(wlcache->findSound("MISSILEFIRESND"), ob);
        break;
    case mechahitlerobj:
    case realhitlerobj:
    case bossobj:
        PlaySoundLocActor(wlcache->findSound("BOSSFIRESND"), ob);
        break;
    case schabbobj:
        PlaySoundLocActor(wlcache->find("SCHABBSTHROWSND"), ob);
        break;
    case fakeobj:
        PlaySoundLocActor(wlcache->findSound("FLAMETHROWERSND"), ob);
        break;
    default:
        PlaySoundLocActor(wlcache->findSound("NAZIFIRESND"), ob);
    }
}
...
void T_Bite (objtype *ob)
{
    ...
    PlaySoundLocActor(wlcache->findSound("DOGATTACKSND"), ob); // JAB
    ...
    if (dx <= MINACTORDIST) {
        ...
        dy -= TILEGLOBAL;
        if (dy <= MINACTORDIST) {
            if (wluser->US_RndT() < 180) {
                TakeDamage (wluser->US_RndT() >> 4, ob);
                return;
            }
        }
    }
...
{
    wlstate->spawnNewObj(player->tilex,player->tiley+1,&s_bjrun1);
    newobj->x = player->x;
    newobj->y = player->y;
...
void T_BJYell (objtype *ob)
{
    PlaySoundLocActor(wlcache->findSound("YEAHSND"), ob);  // JAB
}
...
qint8 CheckPosition (objtype *ob)
{
    int     x,y,xl,yl,xh,yh;
    objtype *check;
...
void    A_StartDeathCam (objtype *ob)
{
    ...
    unsigned fadeheight = viewsize != 21 ? g_Image->height() - scaleFactor*STATUSLINES : g_Image->height();
    wlrender->barScaledCoord(0, 0, g_Image->width(), fadeheight, bordercol);
    ...
    wlinput->userInput(300);
    wlstate->newState(player,&s_deathcam);
    ...
    player->tilex = (quint16)(player->x >> TILESHIFT);         // scale to tile values
    player->tiley = (quint16)(player->y >> TILESHIFT);
    DrawPlayBorder ();
    fizzlein = true;
    ...
    {
    case schabbobj:
        wlstate->newState(ob,&s_schabbdeathcam);
        break;
    case realhitlerobj:
        wlstate->newState(ob,&s_hitlerdeathcam);
        break;
    case giftobj:
        wlstate->newState(ob,&s_giftdeathcam);
        break;
    case fatobj:
        wlstate->newState(ob,&s_fatdeathcam);
        break;
    default:
        break;
    }
